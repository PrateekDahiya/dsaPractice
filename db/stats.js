...
async function getStats(userId) {
  ...
  try {
    ...
    const perDay = perDayRows.map(r => ({ date: r.d, count: r.cnt }));
    // perDay already has distinct dates and counts — reuse for streaks/calendar
    const countMap = {};
    for (const r of perDayRows) countMap[r.d] = r.cnt;
    const distinctDates = perDayRows.map(r => r.d).sort();
